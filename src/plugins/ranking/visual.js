...
import { external_colors } from "./colors";
import "./color_ranges";
import { external_imgs } from "./imgs";
...
export function run() {
  d3.select("#play").on("click", getRemoteFile);
}
...
function getRemoteFile(filePath) {
  ...
  fetch(filePath).then(res => {
    ...
    var r = new FileReader();
    r.readAsText(b, config.encoding);
    r.onload = function () {
      //读取完成后，数据保存在对象的result属性中
      var data = d3.csvParse(this.result);
      console.log(data)
...
function draw(data) {
  ...
  function getColor(d) {
    ...
    if (d[divide_color_by] in config.color)
      ...
      const generateRandomColor = (str) => {
        return product_palette[MyHash(str)%product_palette.length];
      };
      return generateRandomColor(d[divide_color_by]);
    }
  }
  ...
  svg.selectAll("*").remove(); // 移除svg内部节点
